...
def val_epoch(model, loader, epoch, acc_func, args, model_inferer=None, post_label=None, post_pred=None):
    model.eval()
    # model_inferer=model_inferer.cuda(args.rank)
    model=model.cuda(args.rank)
    start_time = time.time()
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
...
def save_checkpoint(model, epoch, args, filename="model.pt", best_acc=0, optimizer=None, scheduler=None):
    ...
    filename = args.model+"_"+str(args.seed)+"_"+filename
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            if args.rank == 0:
                ...
                if val_avg_acc > val_acc_max:
                    ...
                    if args.rank == 0 and args.logdir is not None and args.save_checkpoint:
                        filename = args.model+"_"+str(args.seed)+"_"+"model.pth"
                        file = os.path.join(args.logdir, filename)
            ...
            if args.rank == 0 and args.logdir is not None and args.save_checkpoint:
                ...
                if b_new_best:
                    ...
                    filename = args.model+"_"+str(args.seed)+"_"+str(epoch)+"_"+"model_final.pth"
                    file = os.path.join(args.logdir, filename)
                    torch.save(model, file)
                    # shutil.copyfile(os.path.join(args.logdir, filename), os.path.join(args.logdir, "model.pt"))
